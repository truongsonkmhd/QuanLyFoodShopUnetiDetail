...
public class PBSanPhamDao implements ChiTietInterface<PhienBanSanPhamDTO> {

    public static PBSanPhamDao getInstance() {
        return new PBSanPhamDao();
    }
    ...
    public int insert(ArrayList<PhienBanSanPhamDTO> t) {
        ...
        for (int i = 0; i < t.size(); i++) {
            try {
                ...
                String sql = "INSERT INTO `phienbansanpham`(`maphienbansp`, `masp`, `gianhap`, `giaxuat`,`soluongton`) VALUES (?,?,?,?,?)";
    ...
    public int insert(PhienBanSanPhamDTO t) {
        ...
        try {
            Connection con = (Connection) JDBCUtil.getConnection();
            String sql = "INSERT INTO `phienbansanpham`(`maphienbansp`, `masp`, `gianhap`, `giaxuat`) VALUES (?,?,?,?,?)";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
    ...
    public int update(PhienBanSanPhamDTO ch) {
        ...
        try {
            Connection con = (Connection) JDBCUtil.getConnection();
            String sql = "UPDATE `phienbansanpham` SET `gianhap`=?,`giaxuat`=? WHERE maphienbansp=?";
            PreparedStatement pst = (PreparedStatement) con.prepareStatement(sql);
    ...
    public ArrayList<PhienBanSanPhamDTO> selectAll(String t) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                PhienBanSanPhamDTO ch = new PhienBanSanPhamDTO(maphienbansp, masp, gianhap, giaxuat, soluongton);
                result.add(ch);
            }
            JDBCUtil.closeConnection(con);
    ...
    public PhienBanSanPhamDTO selectById(int mapb) {
        ...
        try {
            ...
            while (rs.next()) {
                ...
                int giaxuat = rs.getInt("giaxuat");
                int soluongton = rs.getInt("soluongton");
                ch = new PhienBanSanPhamDTO(maphienbansp, masp, gianhap, giaxuat, soluongton);
            }
            JDBCUtil.closeConnection(con);
        } catch (SQLException e) {
        }
